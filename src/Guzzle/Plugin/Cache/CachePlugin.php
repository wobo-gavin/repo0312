...
class CachePlugin implements EventSubscriberInterface
{
    ...
    {
        if (!$response->hasHeader('X-/* Replaced /* Replaced /* Replaced Guzzle */ */ */-Cache')) {
            $response->setHeader('X-/* Replaced /* Replaced /* Replaced Guzzle */ */ */-Cache', "key={$cacheKey}");
        }
        ...
        if ($this->debugHeaders) {
            if ($request->getParams()->get('cache.lookup') === true) {
                $response->addHeader('X-Cache-Lookup', 'HIT from /* Replaced /* Replaced /* Replaced Guzzle */ */ */Cache');
            } else {
                $response->addHeader('X-Cache-Lookup', 'MISS from /* Replaced /* Replaced /* Replaced Guzzle */ */ */Cache');
            }
            if ($request->getParams()->get('cache.hit') === true) {
                $response->addHeader('X-Cache', 'HIT from /* Replaced /* Replaced /* Replaced Guzzle */ */ */Cache');
            } elseif ($request->getParams()->get('cache.hit') === 'error') {
                $response->addHeader('X-Cache', 'HIT_ERROR from /* Replaced /* Replaced /* Replaced Guzzle */ */ */Cache');
            } else {
                $response->addHeader('X-Cache', 'MISS from /* Replaced /* Replaced /* Replaced Guzzle */ */ */Cache');
            }
        }
        ...
        if ($response->isFresh() === false) {
            ...
            if ($request->getParams()->get('cache.hit') === 'error') {
                $response->addHeader(
                    'Warning',
                    sprintf('111 /* Replaced /* Replaced /* Replaced Guzzle */ */ */Cache/%s "Revalidation failed"', Version::VERSION)
                );
            }
        }
    }
