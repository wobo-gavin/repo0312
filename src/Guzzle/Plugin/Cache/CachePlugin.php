        ...
        if ($cachedData = $this->storage->fetch($hashKey)) {
            ...
            if ($this->canResponseSatisfyRequest($request, $response)) {
                ...
                if ($response->isFresh() === false) {
                    $response->addHeader(
                        'Warning',
                        sprintf('110 /* Replaced /* Replaced /* Replaced Guzzle */ */ */Cache/%s "Response is stale"', Version::VERSION)
                    );
                }
                if (!$response->hasHeader('X-/* Replaced /* Replaced /* Replaced Guzzle */ */ */-Cache')) {
                    $response->setHeader('X-/* Replaced /* Replaced /* Replaced Guzzle */ */ */-Cache', "key={$hashKey}");
                }
