...
namespace /* Replaced /* Replaced /* Replaced Guzzle */ */ */\Plugin\Async;
...
class AsyncPlugin implements EventSubscriberInterface
{
    ...
    public function onRequestTimeout(Event $event)
    {
        if ($event['exception'] instanceof CurlException) {
            $event['request']->setResponse(new Response(200, array(
                'X-/* Replaced /* Replaced /* Replaced Guzzle */ */ */-Async' => 'Did not wait for the response'
            )));
        }
    }

    /**
     * Event emitted when a request completes because it took less than 1ms. Add an X-/* Replaced /* Replaced /* Replaced Guzzle */ */ */-Async header to notify the
    ...
    public function onRequestSent(Event $event)
    {
        ...
        $event['request']->getResponse()->setHeader('X-/* Replaced /* Replaced /* Replaced Guzzle */ */ */-Async', 'Did not wait for the response');
    }
}
