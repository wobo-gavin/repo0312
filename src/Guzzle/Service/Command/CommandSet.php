...
class CommandSet implements \IteratorAggregate, \Countable
{
    ...
    {
        // Keep a list of all commands with no /* Replaced /* Replaced /* Replaced client */ */ */
        $invalid = array_filter($this->commands, function($command) {
            return !$command->getClient();
        });

        // If any commands do not have a /* Replaced /* Replaced /* Replaced client */ */ */, then throw an exception
        if (count($invalid)) {
            $e = new CommandSetException('Commands found with no associated /* Replaced /* Replaced /* Replaced client */ */ */');
        ...
        if (count($this->commands)) {
            $multis = array();
            // Prepare each request and send out /* Replaced /* Replaced /* Replaced client */ */ */ notifications
