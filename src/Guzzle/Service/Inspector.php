...
class Inspector
{
    ...
    public function validateConfig(array $params, Collection $config, $strict = true)
    {
        ...
        foreach ($params as $name => $arg) {
            ...
            if (is_scalar($config->get($name)) && strpos($config->get($name), '{{') !== false) {
                $config->set($name, /* Replaced /* Replaced /* Replaced Guzzle */ */ */::inject($config->get($name), $config));
            }
    ...
    /**
     * Get a constraint by name: e.g. "type:/* Replaced /* Replaced /* Replaced Guzzle */ */ */\Common\Collection"
     *
