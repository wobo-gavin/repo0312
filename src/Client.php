...
class Client implements ClientInterface
{
     ...
     *
     * If running /* Replaced /* Replaced /* Replaced Guzzle */ */ */ in a phar, the cacert is copied to one of the
    ...
    public static function getDefaultBundle()
    {
        ...
        if (!$dest) {
            ...
            if (isset($_SERVER['GUZZLE_CA_EXTRACT_DIR'])) {
                ...
                $dest = $_SERVER['GUZZLE_CA_EXTRACT_DIR'] . '//* Replaced /* Replaced /* Replaced guzzle */ */ */-cacert.pem';
            } elseif (isset($_SERVER['HOME'])) {
                // Copy to the user's home directory if available
                $dest = $_SERVER['HOME'] . '//* Replaced /* Replaced /* Replaced guzzle */ */ */-cacert.pem';
            } else {
                // Last effort: Copy to the system's temp directory
                $dest = sys_get_temp_dir() . '//* Replaced /* Replaced /* Replaced guzzle */ */ */-cacert.pem';
            }
        }
