...
/**
 * @covers \/* Replaced Guzzle */Http\MessageFormatter
 */
class MessageFormatterTest extends TestCase
{
    ...
    {
        $request = new Request('PUT', '/', ['x-test' => 'abc'], /* Replaced Psr7 */\Utils::streamFor('foo'));
        $response = new Response(200, ['X-Baz' => 'Bar'], /* Replaced Psr7 */\Utils::streamFor('baz'));
        $err = new RequestException('Test', $request, $response);

        return [
            ['{request}', [$request], /* Replaced Psr7 */\Message::toString($request)],
            ['{response}', [$request, $response], /* Replaced Psr7 */\Message::toString($response)],
            ['{request} {response}', [$request, $response], /* Replaced Psr7 */\Message::toString($request) . ' ' . /* Replaced Psr7 */\Message::toString($response)],
            // Empty response yields no value
            ['{request} {response}', [$request], /* Replaced Psr7 */\Message::toString($request) . ' '],
