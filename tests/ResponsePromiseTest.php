...
/**
 * @covers /* Replaced /* Replaced /* Replaced Guzzle */ */ */Http\ResponsePromise
 */
class ResponsePromiseTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testCreatesFromFulfilledPromise()
    {
        ...
        $this->assertInstanceOf('/* Replaced /* Replaced /* Replaced Guzzle */ */ */Http\FulfilledResponse', $p2);
    ...
    public function testCreatesFromPendingPromise()
    {
        ...
        $p = new Promise();
        $p2 = ResponsePromise::fromPromise($p);
        $this->assertInstanceOf('/* Replaced /* Replaced /* Replaced Guzzle */ */ */Http\ResponsePromise', $p2);
        $p->resolve($r);
        $this->assertEquals(200, $p2->getStatusCode());
    }
    ...
    public function testCreatesByExtractingValueFromRejectedPromise()
    {
        ...
        $p->reject($e);
        $p2 = ResponsePromise::fromPromise($p);
        $this->assertInstanceOf('/* Replaced /* Replaced /* Replaced Guzzle */ */ */Http\RejectedResponse', $p2);
        try {
            $p2->getStatusCode();
            $this->fail();
        } catch (\Exception $e2) {
    ...
    {
        $p = $this->getMockBuilder('/* Replaced /* Replaced /* Replaced Guzzle */ */ */Http\Promise\Promise')
            ->setMethods(['getState'])
            ->getMock();
        $p->expects($this->any())
            ->method('getState')
