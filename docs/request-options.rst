...
You can customize requests created and transferred by a /* Replaced client */ using
...
All of the following examples use the following /* Replaced client */:

.. code-block:: php

    $/* Replaced client */ = new /* Replaced Guzzle */Http\Client(['base_uri' => 'http://httpbin.org']);
...
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::ALLOW_REDIRECTS``

Set to ``false`` to disable redirects.

.. code-block:: php

    $res = $/* Replaced client */->request('GET', '/redirect/3', ['allow_redirects' => false]);
...
.. code-block:: php
    ...
    $res = $/* Replaced client */->request('GET', '/redirect/3');
    echo $res->getStatusCode();
    // 200
...
- track_redirects: (bool) When set to ``true``, each redirected URI and status
  code encountered will be tracked in the ``X-/* Replaced Guzzle */-Redirect-History`` and
  ``X-/* Replaced Guzzle */-Redirect-Status-History`` headers respectively. All URIs and
  status codes will be stored in the order which the redirects were encountered.

  Note: When tracking redirects the ``X-/* Replaced Guzzle */-Redirect-History`` header will
  exclude the initial request's URI and the ``X-/* Replaced Guzzle */-Redirect-Status-History``
...
.. code-block:: php
    ...
    echo $res->getHeaderLine('X-/* Replaced Guzzle */-Redirect-History');
    // http://first-redirect, http://second-redirect, etc...

    echo $res->getHeaderLine('X-/* Replaced Guzzle */-Redirect-Status-History');
    // 301, 302, etc...

.. warning::

    This option only has an effect if your handler has the
    ``/* Replaced Guzzle */Http\Middleware::redirect`` middleware. This middleware is added
    by default when a /* Replaced client */ is created with no handler, and is added by
    default when creating a handler with ``/* Replaced Guzzle */Http\HandlerStack::create``.
...
:Types:
        ...
        - null
:Default: None
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::AUTH``
...
.. code-block:: php
    ...
    $/* Replaced client */->request('GET', '/get', ['auth' => ['username', 'password']]);
...
.. code-block:: php
    ...
    $/* Replaced client */->request('GET', '/get', [
...
:Default: None
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::BODY``
...
- string
  ...
  .. code-block:: php
      ...
      $/* Replaced client */->request('PUT', '/put', ['body' => 'foo']);
...
- resource returned from ``fopen()``
  ...
  .. code-block:: php
      ...
      $/* Replaced client */->request('PUT', '/put', ['body' => $resource]);

- ``Psr\Http\Message\StreamInterface``

  .. code-block:: php

      // You can send requests that use a /* Replaced Guzzle */ stream object as the body
      $stream = /* Replaced Guzzle */Http\/* Replaced Psr7 */\Utils::streamFor('contents...');
      $/* Replaced client */->request('POST', '/post', ['body' => $stream]);
...
:Summary: Set to a string to specify the path to a file containing a PEM
        formatted /* Replaced client */ side certificate. If a password is required, then set to
...
:Types:
        ...
        - array
:Default: None
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::CERT``

.. code-block:: php

    $/* Replaced client */->request('GET', '/', ['cert' => ['/path/server.pem', 'password']]);
...
:Types: ``/* Replaced Guzzle */Http\Cookie\CookieJarInterface``
:Default: None
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::COOKIES``

You must specify the cookies option as a
``/* Replaced Guzzle */Http\Cookie\CookieJarInterface`` or ``false``.

.. code-block:: php

    $jar = new \/* Replaced Guzzle */Http\Cookie\CookieJar();
    $/* Replaced client */->request('GET', '/get', ['cookies' => $jar]);

.. warning::

    This option only has an effect if your handler has the
    ``/* Replaced Guzzle */Http\Middleware::cookies`` middleware. This middleware is added
    by default when a /* Replaced client */ is created with no handler, and is added by
    default when creating a handler with ``/* Replaced Guzzle */Http\default_handler``.

.. tip::

    When creating a /* Replaced client */, you can set the default cookie option to ``true``
    to use a shared cookie session associated with the /* Replaced client */.
...
:Types: float
:Default: ``0``
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::CONNECT_TIMEOUT``

.. code-block:: php

    // Timeout if the /* Replaced client */ fails to connect to the server in 3.14 seconds.
    $/* Replaced client */->request('GET', '/delay/5', ['connect_timeout' => 3.14]);
...
:Types:
        ...
        - ``fopen()`` resource
:Default: None
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::DEBUG``

.. code-block:: php

    $/* Replaced client */->request('GET', '/get', ['debug' => true]);
...
::
    ...
    *   Trying 107.21.213.98... * Connected to httpbin.org (107.21.213.98) port 80 (#0)
    > GET /get HTTP/1.1
    Host: httpbin.org
    User-Agent: /* Replaced Guzzle *//4.0 curl/7.21.4 PHP/5.5.7
...
:Types:
        ...
        - bool
:Default: ``true``
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::DECODE_CONTENT``
...
or deflated response will be decoded by /* Replaced Guzzle */.
...
.. code-block:: php
    ...
    $/* Replaced client */->request('GET', '/foo.js', [
...
.. code-block:: php
    ...
    $/* Replaced client */->request('GET', '/foo.js', ['decode_content' => 'gzip']);
...
:Types:
    ...
    - float
:Default: null
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::DELAY``
...
:Types:
    ...
    - integer
:Default: ``1048576``
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::EXPECT``
...
By default, /* Replaced Guzzle */ will add the "Expect: 100-Continue" header when the size of
...
.. note::
    ...
    implemented by /* Replaced Guzzle */ HTTP handlers used by a /* Replaced client */.
...
:Types: string
:Default: null
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::FORCE_IP_RESOLVE``

.. code-block:: php

    // Force ipv4 protocol
    $/* Replaced client */->request('GET', '/foo', ['force_ip_resolve' => 'v4']);

    // Force ipv6 protocol
    $/* Replaced client */->request('GET', '/foo', ['force_ip_resolve' => 'v6']);
...
:Types: array
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::FORM_PARAMS``
...
.. code-block:: php
    ...
    $/* Replaced client */->request('POST', '/post', [
        'form_params' => [
            'foo' => 'bar',
            'baz' => ['hi', 'there!']
        ]
...
:Types: array
:Defaults: None
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::HEADERS``
...
Headers may be added as default options when creating a /* Replaced client */. When headers
...
to the /* Replaced client */ in the ``send()`` and ``sendAsync()`` methods, and requests
created by the /* Replaced client */ (e.g., ``request()`` and ``requestAsync()``).

.. code-block:: php

    $/* Replaced client */ = new /* Replaced Guzzle */Http\Client(['headers' => ['X-Foo' => 'Bar']]);
    ...
    // from being applied.
    $/* Replaced client */->request('GET', '/get', ['headers' => ['X-Foo' => 'test']]);

    // Will disable adding in default headers.
    $/* Replaced client */->request('GET', '/get', ['headers' => null]);
    ...
    use /* Replaced Guzzle */Http\/* Replaced Psr7 */\Request;
    $request = new Request('GET', 'http://foo.com', ['X-Foo' => 'test']);
    $/* Replaced client */->send($request, ['headers' => ['X-Foo' => 'overwrite']]);
...
:Types: bool
:Default: ``true``
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::HTTP_ERRORS``

.. code-block:: php

    $/* Replaced client */->request('GET', '/status/500');
    // Throws a /* Replaced Guzzle */Http\Exception\ServerException

    $res = $/* Replaced client */->request('GET', '/status/500', ['http_errors' => false]);
    echo $res->getStatusCode();
    // 500
...
.. warning::
    ...
    This option only has an effect if your handler has the
    ``/* Replaced Guzzle */Http\Middleware::httpErrors`` middleware. This middleware is added
    by default when a /* Replaced client */ is created with no handler, and is added by
    default when creating a handler with ``/* Replaced Guzzle */Http\default_handler``.
...
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::IDN_CONVERSION``

.. code-block:: php

    $/* Replaced client */->request('GET', 'https://яндекс.рф');
    // яндекс.рф is translated to xn--d1acpjx3f.xn--p1ai before passing it to the handler

    $res = $/* Replaced client */->request('GET', 'https://яндекс.рф', ['idn_conversion' => false]);
...
:Default: None
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::JSON``

.. code-block:: php

    $response = $/* Replaced client */->request('PUT', '/put', ['json' => ['foo' => 'bar']]);
...
.. code-block:: php
    ...
    // options to the /* Replaced client */ constructor.
    $response = $/* Replaced client */->request('PUT', '/put', [
        'json'    => ['foo' => 'bar'],
...
:Types: array
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::MULTIPART``
...
:Types: - callable
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::ON_HEADERS``
...
be rejected with a ``/* Replaced Guzzle */Http\Exception\RequestException`` that wraps the
...
.. code-block:: php
    ...
    $/* Replaced client */->request('GET', 'http://httpbin.org/stream/1024', [
...
:Summary: ``on_stats`` allows you to get access to transfer statistics of a
    ...
    associated with your /* Replaced client */. ``on_stats`` is a callable that is invoked
    ...
    to send the request.
:Types: - callable
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::ON_STATS``

The callable accepts a ``/* Replaced Guzzle */Http\TransferStats`` object.
...
:Types: - callable
:Default: None
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::PROGRESS``
...
.. code-block:: php
    ...
    $result = $/* Replaced client */->request(
        'GET',
        '/',
        [
            'progress' => function(
...
:Types:
    ...
    - array
:Default: None
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::PROXY``

Pass a string to specify a proxy for all protocols.

.. code-block:: php

    $/* Replaced client */->request('GET', '/', ['proxy' => 'tcp://localhost:8125']);
...
.. note::
    ...
    /* Replaced Guzzle */ will automatically populate this value with your environment's
...
:Types:
    ...
    - string
:Default: None
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::QUERY``

.. code-block:: php

    // Send a GET request to /get?foo=bar
    $/* Replaced client */->request('GET', '/get', ['query' => ['foo' => 'bar']]);
...
.. code-block:: php
    ...
    // Send a GET request to /get?foo=bar
    $/* Replaced client */->request('GET', '/get?abc=123', ['query' => ['foo' => 'bar']]);
...
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::SINK``
...
.. code-block:: php
    ...
    $/* Replaced client */->request('GET', '/stream/20', ['sink' => '/path/to/file']);
...
.. code-block:: php
    ...
    $/* Replaced client */->request('GET', '/stream/20', ['sink' => $resource]);
...
.. code-block:: php
    ...
    $resource = fopen('/path/to/file', 'w');
    $stream = /* Replaced Guzzle */Http\/* Replaced Psr7 */\Utils::streamFor($resource);
    $/* Replaced client */->request('GET', '/stream/20', ['save_to' => $stream]);
...
:Types:
        ...
        - array
:Default: None
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::SSL_KEY``
...
:Types: bool
:Default: ``false``
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::STREAM``

.. code-block:: php

    $response = $/* Replaced client */->request('GET', '/stream/20', ['stream' => true]);
...
.. note::
    ...
    a /* Replaced client */. This option might not be supported by every HTTP handler, but the
...
:Types: bool
:Default: none
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::SYNCHRONOUS``
...
:Types:
    ...
    - string
:Default: ``true``
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::VERIFY``

.. code-block:: php

    // Use the system's CA bundle (this is the default setting)
    $/* Replaced client */->request('GET', '/', ['verify' => true]);

    // Use a custom SSL certificate on disk.
    $/* Replaced client */->request('GET', '/', ['verify' => '/path/to/cert.pem']);

    // Disable validation entirely (don't do this!).
    $/* Replaced client */->request('GET', '/', ['verify' => false]);
...
:Types: float
:Default: ``0``
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::TIMEOUT``

.. code-block:: php

    // Timeout if a server does not return a response in 3.14 seconds.
    $/* Replaced client */->request('GET', '/delay/5', ['timeout' => 3.14]);
    // PHP Fatal error:  Uncaught exception '/* Replaced Guzzle */Http\Exception\TransferException'
...
:Types: string, float
:Default: ``1.1``
:Constant: ``/* Replaced Guzzle */Http\RequestOptions::VERSION``

.. code-block:: php

    // Force HTTP/1.0
    $request = $/* Replaced client */->request('GET', '/get', ['version' => 1.0]);
